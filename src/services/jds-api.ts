 ...
 */
export async function getCaseById(idOrSlug: number | string): Promise<CaseDetail> {
  try {
    const response = await apiClient.get<CaseDetail>(`/cases/${idOrSlug}/`);
    return response.data;
  } catch (error) {
    handleApiError(error, `/cases/${idOrSlug}/`);
  }
}
