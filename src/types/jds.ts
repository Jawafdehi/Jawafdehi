...
/** Lifecycle of the proceedings, derived server-side. Distinct from `state`,
 * which is our editorial workflow (DRAFT/IN_REVIEW/PUBLISHED/CLOSED). */
...
/** Which prosecution/adjudication route the case travelled. */
...
/** The closed stage vocabulary. A value outside it does not render. */
export type CaseStageName =
  | 'investigation'
  | 'initial'
  | 'appeal'
  | 'review'
...
/** One pass of a case through one forum.
 *
 * A case is not one date range: a published case runs 12 dockets across 8
 * courts on three tiers, and a Supreme Court writ has no lower instance at all.
 * Every date is AD `YYYY-MM-DD`; either end may be absent (a stage with no
 * `end` has not concluded).
 */
export interface CaseStage {
  stage: CaseStageName;
  start?: string | null;
  end?: string | null;
  /** Canonical court-case @id IRI this stage was heard under. */
  courtcase_iri?: string | null;
  /** The forum, when it is not a court (an investigating body). */
  body?: string | null;
  /** Present only when `stage` is `other`. */
  label?: string | null;
  /** PUBLIC free text (<=500 chars), Nepali-first: why a stage has no end. */
  notes?: string | null;
}
...
export interface Case {
  ...
  /** DEPRECATED alias of `offence_type`, still served this release. */
  case_type: CaseType;
  /** The offence charged — `case_type` renamed. Read this one. */
  offence_type?: CaseType;
  state: CaseState; // Current state in the workflow
  /** Lifecycle of the proceedings, derived server-side. Optional so cached
   * payloads and pre-deploy responses stay type-safe. */
  status?: CaseStatus | null;
  case_track?: CaseTrack | null;
  /** Every pass of the case through every forum. Replaces the single
  ...
  // DEPRECATED read aliases, superseded by `dates.stages` (and by the derived
  // `proceedings_started_on` / `proceedings_decided_on` where one span is all a
  // surface has room for). Optional so nothing new depends on them; do not read
  // them. The label they were rendered under, "मुद्दा मिति / Case date", read to
  // the public as when the corruption happened, which they never were.
